       ...
       public  static Regex separador = new Regex(TokenUnion); // el regex que se usa para separar los tokens 
        ...
        public static void InvalidToken(string input)                              // este metodo revisa si existe algun toque invalido y devuelve el error correspondiente
        {
            ...
            foreach (Match match in matches)
            {
                ...
                {
                    string x = input.Substring(start, match.Index - start);
                    if (!whitespace.IsMatch(x)) throw new Exception($"! LEXICAL ERROR : '{x}' is not a valid token");
                    
                  //  Token x = new Token(match.Index, input.Substring(start, match.Index - start));
                }
            ...
            {
                string x = input.Substring(start);
                if (!whitespace.IsMatch(x)) throw new Exception($"! LEXICAL ERROR : '{x }' is not a valid token");
            }
        }
